 ...
 */
public class ContactSyncEngine extends BaseEngine implements
	...
	public static enum State {
		IDLE, FETCHING_NATIVE_CONTACTS, UPDATING_NATIVE_CONTACTS, FETCHING_SERVER_CONTACTS, UPDATING_SERVER_CONTACTS,
	...
	public static enum Mode {
		NONE, FULL_SYNC_FIRST_TIME, FULL_SYNC, SERVER_SYNC, THUMBNAIL_SYNC, FETCH_NATIVE_SYNC, UPDATE_NATIVE_SYNC
	}
	...
	private final ApplicationCache mCache = mEventCallback
			.getApplicationCache();
	...
	private static class SyncParams {
		 ...
		 * 
		 * @param isFull
		 *            true if full sync, false if only a server sync
		 * @param delay
		 *            in milliseconds before executing the request
		 */
	...
	public static interface IContactSyncObserver {
		 ...
		 * 
		 * @param status
		 *            SUCCESS if the sync was successful, a suitable error code
		 *            otherwise.
		 */
		 ...
		 * 
		 * @param currentState
		 *            What the contact sync engine is currently doing
		 * @param percent
		 *            Percentage complete for the current task
		 */
	 ...
	 * little bit of time before beginning any heavy background work. This will
	 * give the main thread a chance to render its UI.
	 */
	 ...
	 */
	private final NativeContactsApi mNativeContactsApi = NativeContactsApi
			.getInstance();
	 ...
	 * 
	 * @param eventCallback
	 *            Engine-event callback interface allowing engine to report back
	 *            to client on request completion.
	 * @param context
	 *            Context.
	 * @param db
	 *            Handle to People database.
	 * @param processorFactory
	 *            the processor factory
	 */
	public ContactSyncEngine(IEngineEventCallback eventCallback,
			Context context, DatabaseHelper db,
	...
	public void onCreate() {
		...
		PersistSettings setting1 = mDb
				.fetchOption(PersistSettings.Option.FIRST_TIME_SYNC_STARTED);
		PersistSettings setting2 = mDb
				.fetchOption(PersistSettings.Option.FIRST_TIME_SYNC_COMPLETE);
		...
		if (setting3 != null) {
			mFirstTimeNativeSyncComplete = setting3
	 ...
	 * 
	 * @param delay
	 *            the delay in milliseconds from now when the sync should start
	 */
	public void addUiStartServerSync(long delay) {
		...
		synchronized (this) {
			if ((mMode == Mode.FULL_SYNC_FIRST_TIME || mMode == Mode.FULL_SYNC || mMode == Mode.SERVER_SYNC)
				...
				LogUtils.logD("delay=" + delay + ", currentDelay="
						+ currentDelay);
	...
	public void pingUserActivity() {
		...
		long delay;
		synchronized (this) {
			final long currentDelay = System.currentTimeMillis()
					- mLastServerSyncTime.longValue();
			if ((mMode == Mode.FULL_SYNC_FIRST_TIME || mMode == Mode.FULL_SYNC || mMode == Mode.SERVER_SYNC)
					&& mState != State.IDLE) {
				...
				LogUtils.logD("A new sync is already scheduled in "
						+ (USER_ACTIVITY_SERVER_SYNC_TIMEOUT_MS - currentDelay)
						+ " milliseconds");
				return;
			}
		}
	...
	private void startServerContactSyncTimer(long delay) {
		...
		synchronized (this) {
			...
			if (mCurrentTimeout == null
					|| mCurrentTimeout.compareTo(mServerSyncTimeout) > 0) {
	...
	private void startFetchNativeContactSyncTimer() {
		...
		synchronized (this) {
			if (mFetchNativeSyncTimeout == null) {
				LogUtils
						.logI("ContactSyncEngine - will fetch native contacts shortly...");
			}
			mFetchNativeSyncTimeout = System.currentTimeMillis()
					+ NATIVE_CONTACT_SYNC_TIMEOUT_MS;
			if (mCurrentTimeout == null
					|| mCurrentTimeout.compareTo(mFetchNativeSyncTimeout) > 0) {
	...
	private void startUpdateNativeContactSyncTimer() {
		...
		synchronized (this) {
			if (mUpdateNativeSyncTimeout == null) {
				LogUtils
						.logI("ContactSyncEngine - will update native contacts shortly...");
			}
			mUpdateNativeSyncTimeout = System.currentTimeMillis()
					+ NATIVE_CONTACT_SYNC_TIMEOUT_MS;
			if (mCurrentTimeout == null
					|| mCurrentTimeout.compareTo(mUpdateNativeSyncTimeout) > 0) {
	 ...
	 * 
	 * @param processor
	 *            Processor which was created by processor factory.
	 */
	private void startProcessor(BaseSyncProcessor processor) {
		if (mActiveProcessor != null) {
			LogUtils.logE("ContactSyncEngine.startProcessor - Cannot start "
					+ processor.getClass() + ", because the processor "
					+ mActiveProcessor.getClass() + " is running");
	...
	public long getNextRunTime() {
		...
		if (readyToStartServerSync()) {
			if (mFullSyncRequired || mServerSyncRequired
	...
	private void startSyncIfRequired() {
		...
		if (mServerSyncTimeout != null
				&& mServerSyncTimeout.longValue() < currentTimeMs) {
	 ...
	 * 
	 * @param resp
	 *            Response or push message received
	 */
	@Override
	protected void processCommsResponse(DecodedResponse resp) {
		...
		if (resp.mDataTypes != null && resp.mDataTypes.size() > 0) {
			LogUtils.logD("ContactSyncEngine.processCommsResponse: Req ID = "
					+ resp.mReqId + ", type = "
					+ resp.mDataTypes.get(0).getType());
		} else {
			LogUtils.logD("ContactSyncEngine.processCommsResponse: Req ID = "
					+ resp.mReqId + ", type = NULL");
		}
		if (mActiveProcessor != null) {
	...
	private boolean processPushEvent(DecodedResponse resp) {
		...
		if ((dataType == null)
				|| dataType.getType() != BaseDataType.PUSH_EVENT_DATA_TYPE) {
			return false;
		}
		...
		case CONTACTS_CHANGE:
			...
			mServerSyncRequired = true;
			EngineManager.getInstance().getGroupsEngine()
					.addUiGetGroupsRequest(); // fetch
			// the
			// newest
			// groups
			mEventCallback.kickWorkerThread();
	...
	public void startFullSync() {
		...
		if (mFirstTimeNativeSyncComplete) {
			LogUtils
					.logI("ContactSyncEngine.startFullSync - user triggered sync");
	...
	private boolean startFetchNativeContacts() {
		if (Settings.ENABLE_FETCH_NATIVE_CONTACTS) {
			...
			startProcessor(mProcessorFactory.create(
					ProcessorFactory.FETCH_NATIVE_CONTACTS, this, mDb,
					mContext, mCr));
			return true;
		}
		return false;
	...
	private boolean startUpdateNativeContacts() {
		if (Settings.ENABLE_UPDATE_NATIVE_CONTACTS) {
			...
			startProcessor(mProcessorFactory.create(
					ProcessorFactory.UPDATE_NATIVE_CONTACTS, this, mDb, null,
					mCr));
			return true;
		}
		return false;
	...
	private boolean startDownloadServerContacts() {
		if (Settings.ENABLE_SERVER_CONTACT_SYNC) {
			...
			startProcessor(mProcessorFactory.create(
					ProcessorFactory.DOWNLOAD_SERVER_CONTACTS, this, mDb,
					mContext, null));
			return true;
	...
	private boolean startUploadServerContacts() {
		if (Settings.ENABLE_SERVER_CONTACT_SYNC) {
			...
			startProcessor(mProcessorFactory.create(
					ProcessorFactory.UPLOAD_SERVER_CONTACTS, this, mDb,
					mContext, null));
			return true;
		}
		return false;
	 ...
	 *            stop the sync.
	 * @param failureList
	 *            Contains a list of sync failure information which can be used
	 *            as a summary at the end. Otherwise should be an empty string.
	 * @param data
	...
	public void onProcessorComplete(ServiceStatus status, String failureList,
		...
		if (status != ServiceStatus.SUCCESS) {
			LogUtils
					.logE("ContactSyncEngine.onProcessorComplete - Failed during "
							+ mState + " with error " + status);
			completeSync(status);
			return;
		}
		...
		default:
			LogUtils
					.logE("ContactSyncEngine.onProcessorComplete - Unexpected mode: "
							+ mMode);
	...
	private void nextTaskFullSyncFirstTime() {
		...
		default:
			LogUtils
					.logE("ContactSyncEngine.nextTaskFullSyncFirstTime - Unexpected state: "
							+ mState);
			completeSync(ServiceStatus.ERROR_SYNC_FAILED);
		}
	}
	...
	private void nextTaskFullSyncNormal() {
		...
		default:
			LogUtils
					.logE("ContactSyncEngine.nextTaskFullSyncNormal - Unexpected state: "
							+ mState);
			completeSync(ServiceStatus.ERROR_SYNC_FAILED);
		}
	}
	...
	private void nextTaskFetchNativeContacts() {
		...
		default:
			LogUtils
					.logE("ContactSyncEngine.nextTaskFetchNativeContacts - Unexpected state: "
							+ mState);
			completeSync(ServiceStatus.ERROR_SYNC_FAILED);
		}
	}
	...
	private void nextTaskUpdateNativeContacts() {
		...
		default:
			LogUtils
					.logE("ContactSyncEngine.nextTaskUpdateNativeContacts - Unexpected state: "
							+ mState);
			completeSync(ServiceStatus.ERROR_SYNC_FAILED);
		}
	}
	...
	private void nextTaskServerSync() {
		...
		default:
			LogUtils
					.logE("ContactSyncEngine.nextTaskServerSync - Unexpected state: "
							+ mState);
			completeSync(ServiceStatus.ERROR_SYNC_FAILED);
		}
	}
	...
	private void nextTaskThumbnailSync() {
		...
		default:
			LogUtils
					.logE("ContactSyncEngine.nextTaskThumbnailSync - Unexpected state: "
							+ mState);
			completeSync(ServiceStatus.ERROR_SYNC_FAILED);
		}
	}
	...
	private void newState(State newState) {
		...
		LogUtils.logV("ContactSyncEngine.newState: " + oldState + " -> "
				+ mState);
	 ...
	 * 
	 * @param status
	 *            The overall status of the contact sync.
	 */
	private void completeSync(ServiceStatus status) {
		...
		if (mDatabaseChanged) {
			LogUtils
					.logD("ContactSyncEngine.completeSync - Firing Db changed event");
		...
		mUiAgent.sendUnsolicitedUiEvent(ServiceUiRequest.UPDATE_SYNC_STATE,
				null);
	...
	private synchronized void setTimeoutIfRequired() {
		...
		if (mCurrentTimeout == null
				|| (mServerSyncTimeout != null && mServerSyncTimeout
	...
	public void onDatabaseChanged() {
		...
		if (mLastDbUpdateTime == null
			...
			LogUtils
					.logD("ContactSyncEngine.onDatabaseChanged - Updating UI...");
			mDatabaseChanged = false;
	 ...
	 * 
	 * @param timeout
	 *            Timeout value based on current time in milliseconds
	 */
	@Override
	...
	public void setSyncStatus(final SyncStatus syncStatus) {
		...
		if (mState != State.IDLE
				&& syncStatus.getProgress() != mCurrentProgressPercent) {
	 ...
	 * 
	 * @param value
	 *            New value to the flag. True indicates that first time sync has
	 *            been started. The flag is never set to false again by the
	 ...
	 * @param value
	 *            New value to the flag. True indicates that first time sync has
	 *            been completed. The flag is never set to false again by the
	 *            engine, it will be only set to false when a remove user data
	 *            is done (and the database is deleted).
	 * @return SUCCESS or a suitable error code if the database could not be
	 *         updated.
	 ...
	 * 
	 * @param msg
	 *            The message indicating the type of event
	 */
	private void processDbMessage(Message message) {
		final ServiceUiRequest event = ServiceUiRequest
				.getUiEvent(message.what);
		switch (event) {
		case DATABASE_CHANGED_EVENT:
			if (message.arg1 == DatabaseHelper.DatabaseChangeType.CONTACTS
					.ordinal()
					&& message.arg2 == 0) {
				LogUtils
						.logV("ContactSyncEngine.processDbMessage - Contacts have changed");
	 ...
	 * 
	 * @param currentState
	 *            Current sync task being processed
	 * @param percent
	...
	public void onChange() {
		...
		LogUtils
				.logD("ContactSyncEngine.onChange(): changes detected on native side.");
	...
	/**
	 * This method adds a request to send friend request. The request is added
	 * to the UI request and processed when the engine is ready
	 * 
	 * @param data
	 *            contains the contact ID of the person to whom request is to be
	 *            send and message to be sent
	 * @return void
	 */
	...
	/**
	 * This method handles Server response to send request. The response is
	 * Success if friend request is sent successfully and -1 on failure and on
	 * any wrong input Bad parameter is returned.
	 * 
	 * @param response
	...
	/**
	 * This method adds a request to get friend request. The request is added to
	...
	private void receivedFriendReqIds(List<BaseDataType> data) {
		...
		if (requests.size() != 0) {
			mCache.setReqId(requests.get(0).mRequestId,
					requests.get(0).mUserProfile.userID,
					requests.get(0).mUserProfile.contactID);

		}
	}

	/**
	 * This method adds a request to reject friend requests. The request is
	 * added to the UI request and processed when the engine is ready.
	 * 
	 * @param reqIdList
	 *            contains new friend request id list
	 * @return void
	 */
	 ...
	 * 
	 * @param reqIds
	 *            contains id of the friends to be rejected
	 * @return void
	 */
	...
	/***
	 * Handle Server response to reject a friend request. The response is
	 * Success if request is rejected successfully and -1 on failure and on any
	 * wrong input Bad parameter is returned.
	 * 
	 * @param response
			...
			for (int it = 0; it < data.size(); it++) {
				...
				if (reqIdList.get(0) != -1) {
					LogUtils.logI(reqIdList.size()
							+ " user/s' friend request rejected");
					/*
	...
	/**
	 * This method adds a request to remove friend. The request is added to the
	 * UI request and processed when the engine is ready.
	 * 
	 * @param userIdList
	 *            contains list of user id
	 * @return void
	 */
	 ...
	 * 
	 * @param usrIds
	 *            contains the user id of the friend to be removed
	 * @return void
	 */
	...
	/**
	 * This method adds a request to approve pending friend requests. The
	 * request is added to the UI request and processed when the engine is
	 * ready.
	 * 
	 * @param reqIdList
	 *            The new invites request id list
	 * @return void
	 */
	 ...
	 * 
	 * @param reqIds
	 *            : contains the new invites request id list
	 * @return void
	 */
