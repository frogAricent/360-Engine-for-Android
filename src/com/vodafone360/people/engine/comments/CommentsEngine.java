...
//
//	 private Hashtable<Integer, String> mActiveRequests = new Hashtable<Integer, String>();
//
...
//	   
	 /**
	     ...
	     * POST_COMMENT: Post comment for an Entity,
	     * DELETE_COMMENT: Delete comment of an Entity
	     */
	    private enum State {
	        IDLE,
	        POST_COMMENT,
...
//        mContext = context;
	 ...
	 /**
     * Return next run time for CommentsEngine. Determined by whether we have
    ...
    /**
     * Add request to post comments. The request is added to the UI
     * request and processed when the engine is ready.
     * 
     * @param commentsList The list of comments to be posted
     */
	...
	/**
     * Add request to delete comments. The request is added to the UI
     * request and processed when the engine is ready.
     * 
     * @param data Bundle contains the list of comments to be deleted 
     * 				and the owner ID of the comments
     */
	...
	public void addUiUpdateCommentRequest(List <Comment> commentsList) {
	 ...
	 /**
     * Issue request to Delete comments. (Request is not issued if
	...
	private void startDeleteComment(Object data) {
	 ...
	 /**
     * Issue request to post comments. (Request is not issued if
     * there is currently no connectivity).
     * 
     * @param commentsList The list of comments to be added.
     */
    ...
    */
	...
	private void handleDeleteCommentResponse(List<BaseDataType> data) {
		LogUtils.logD("CommentsEngine.handlePostCommentResponse()");
		 ...
		 ServiceStatus errorStatus = getResponseStatus(BaseDataType.COMMENTS_RESPONSE_DATATYPE, data);
	        if (errorStatus == ServiceStatus.SUCCESS) {
	        	mComment.clear();
	               ...
	               for (BaseDataType item : data) {
	                   if (BaseDataType.COMMENTS_RESPONSE_DATATYPE == item.getType()) {
	                       ...
	                       LogUtils.logD("Comment id: " + ((CommentsResponse)item).mCommentIdList);
	                   } else {
	                	   LogUtils.logE("Comemnts Engine: handleDeleteCommentResponse Unexpected response");
	        	...
	        	LogUtils.logE("CommentsEngine handleDeleteCommentResponse error status: " + errorStatus);
	        }
	}
	...
	private void handlePostCommentResponse(List<BaseDataType> data) {
		LogUtils.logD("CommentsEngine.handlePostCommentResponse()");
//		 ServiceStatus errorStatus = genericHandleResponseType("CommentsResponse", data);
		 ServiceStatus errorStatus = getResponseStatus(BaseDataType.COMMENTS_RESPONSE_DATATYPE, data);
	        ...
	        if (errorStatus == ServiceStatus.SUCCESS) {
	        	mCommentsResponse.clear();
	               for (BaseDataType item : data) {
	                   if (BaseDataType.COMMENTS_RESPONSE_DATATYPE == item.getType()) {
	                	   ...
	                	   LogUtils.logE("CommentsEngine: handlePostCommentResponse Unexpected response");
	                       completeUiRequest(ServiceStatus.ERROR_UNEXPECTED_RESPONSE);
	                       return;
	                   }
	               }
	        	...
	        	LogUtils.logE("CommentsEngine handlePostCommentResponse error status: " + errorStatus);
	        }
	}
	...
	private void handleGetCommentResponse(List<BaseDataType> data) {
		LogUtils.logD("ContentsEngine.handleGetCommentResponse()");
		 ...
		 ServiceStatus errorStatus = getResponseStatus(BaseDataType.COMMENT_LIST_DATATYPE, data);
	        if (errorStatus == ServiceStatus.SUCCESS) {
	               for (BaseDataType item : data) {
	   	        		mCommentsResponse.clear();
	                   if (BaseDataType.COMMENT_LIST_DATATYPE == item.getType()) {
	                	   mCommentListResponse = (CommentListResponse)item;
	                       LogUtils.logD("Comment id: " + mCommentListResponse.mCommentList);
	                   } else {
	                	   LogUtils.logE("CommentsEngine handleGetCommentResponse Unexpected response: " + item.getType());
	                       return;
	                   }
	               }
	            }
	        else{
	        	LogUtils.logE("CommentsEngine handleGetCommentResponse error status: " + errorStatus.name());
	        }
	        	...
	        	mCommentsResponse.clear();
	               for (BaseDataType item : data) {
	                   if (BaseDataType.COMMENTS_RESPONSE_DATATYPE == item.getType()) {
	                	   ...
	                	   LogUtils.logE("CommentsEngine handleUpdateCommentResponse Unexpected response");
	                       completeUiRequest(ServiceStatus.ERROR_UNEXPECTED_RESPONSE);
	                       return;
	                   }
	               }
